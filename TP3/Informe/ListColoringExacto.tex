...
\begin{itemize}
	...
	\item $c/2$: es la cantidad máxima de colores que puede tener un nodo y que está dada por la entrada del problema.
\end{itemize}
...
\IF {$count ==$ cantidad de nodos del \texttt{grafo}}
	\STATE Llamar a \texttt{2ListColoring} para resolver \texttt{grafo2colores} \COMMENT{$\mathcal{O}(n^2 \log{n})$} %complejidad del 1! revisar
	\STATE Retornar 
\ENDIF
...
\WHILE[$\mathcal{O}(c/2)$ en cantidad de ejecuciones.]{Haya próximo en la lista de colores}
	...
	\IF{Hay próximo en la lista de colores}
		\STATE $color2 = $ Próximo en la lista \COMMENT{$\mathcal{O}(1)$}
